...
def extract_trading_action(md_file_path: str) -> str:
    ...
    try:
        ...
        # 匹配多种可能的操作方向格式
        patterns = [
            r'\*\*操作方向\*\*[：:]\s*(.+?)(?:\s{2,}|\n)',  # 格式1: **操作方向**：买入（markdown粗体，值后有多个空格或换行）
            r'\*\*操作方向：(.+?)\*\*',  # 格式2: **操作方向：买入（轻仓博弈反弹）**
            r'\*\*操作方向\*\*\s*\|\s*(.+?)\s*\|',  # 格式3: | **操作方向** | ✅ 买入（轻仓做反弹） |
            r'\*\*操作方向\*\*[：:]\s*\*\*(.+?)\*\*',  # 格式4: **操作方向**：**观望**
            r'\*\*[^\*]*?操作方向\*\*[：:]\s*\*\*(.+?)\*\*',  # 格式5: **📊 操作方向**：**买入**（带表情符号，值也在双星号中）
            r'操作方向\*\*[：:]\s*(.+?)(?:\s{2,}|\n)',  # 格式6: 操作方向**：买入（考虑后面的**）
            r'操作方向[：:]\s*(.+?)[\n\r]',  # 格式7: 操作方向：买入（无markdown标记）
            r'\*\*交易建议\*\*[：:]\s*(.+?)[\n\r]',  # 格式8: **交易建议：买入**
        ]
