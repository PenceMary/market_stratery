...
⏱️ 距离开盘：{elapsed_minutes} 分钟(负数表示尚未开盘，具体数值为距离开盘的倒计时，比如-15代表离开盘还有15分钟；正数表示已开盘时长，如果是正数代表目前是在盘中进行数据分析。开盘时间为上午9:30-11:30，下午13:00-15:00，中午11:30-13:00为休市时间，若该数值为240，说明当天已收盘)
...
**7. 交易决策（重点）**
   ...
   📊 **操作方向**：买入/卖出/观望（必须明确选择一个且字段保持完全一致，如果是盘中，尽量采取积极操作，考虑盘中高抛低吸做T，不要过于保守而选择观望。大模型反馈的这段文字格式的必须是："**操作方向**：买入" 或者 "**操作方向**：卖出" 或者 "**操作方向**：观望" ，引号中的内容，不能包含其他任何字段）
